...
namespace SMWaterLevel.DataAccess
{
	...
	public class AccountCategoryTableAdapter : BaseDataAdapter
	{
		
		
		#region Constructers

			public AccountCategoryTableAdapter():base()
            {
                InitInsertCommand();
                InitUpdateCommand();
                InitDeleteCommand();
		...
		protected  void InitInsertCommand()
		{
			...
			InsertCommand.CommandText = @"INSERT INTO AccountCategory(AccountCategoryName,Status) VALUES (@AccountCategoryName,@Status);SELECT ID FROM AccountCategory WHERE (ID = SCOPE_IDENTITY())";
		...
		protected  void InitUpdateCommand()
		{
			...
			UpdateCommand.CommandText = @"UPDATE AccountCategory SET AccountCategoryName = @AccountCategoryName, Status = @Status WHERE (ID = @ID)";
		...
		protected  void InitDeleteCommand()
		{
			...
			DeleteCommand.CommandText = "DELETE FROM AccountCategory WHERE (ID = @ID)";
		...
		protected  void InitCommandCollection()
		{
			...
			((SqlCommand) (CommandCollection[0])).CommandText = "SELECT ID,AccountCategoryName,Status FROM AccountCategory";
            ...
            ((SqlCommand)(CommandCollection[1])).CommandText = "SELECT ID,AccountCategoryName,Status FROM AccountCategory WHERE ID =  @ID";
            ...
            ((SqlCommand)(CommandCollection[2])).CommandText = "SELECT ID,AccountCategoryName,Status FROM AccountCategory WHERE Status =  @Status";
		...
		public DataSet GetAccountCategoryByStatus(string status)
		{
		...
		public int Insert(AccountCategory accountCategory)
		{
	InsertCommand.Parameters["@AccountCategoryName"].Value = accountCategory.AccountCategoryName;
	InsertCommand.Parameters["@Status"].Value = accountCategory.Status;
		...
		public int Update(AccountCategory accountCategory)
		{
	UpdateCommand.Parameters["@ID"].Value = accountCategory.ID;
	UpdateCommand.Parameters["@AccountCategoryName"].Value = accountCategory.AccountCategoryName;
	UpdateCommand.Parameters["@Status"].Value = accountCategory.Status;
        ...
        private AccountCategory DataTableToEntity(DataTable dt)
        {
            ...
            if(Null.IsNotNull(dt)==true && dt.Rows.Count > 0)
            {
                ...
                if (Null.IsNotNull(dt.Rows[0]))
                {
					...
					{
						accountCategory.ID=Convert.ToInt32(dr["ID"]);
					}
					else
					{
						accountCategory.ID=0;
					}
					if (Null.IsNotNull(dr["AccountCategoryName"]))
					{
						accountCategory.AccountCategoryName= Convert.ToString(dr["AccountCategoryName"]);
					}
					else
					{
						accountCategory.AccountCategoryName=string.Empty;
					}
					if (Null.IsNotNull(dr["Status"]))
					{
						accountCategory.Status= Convert.ToString(dr["Status"]);
					}
					else
					{
						accountCategory.Status=string.Empty;
					}

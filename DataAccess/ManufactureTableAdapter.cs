...
namespace SMWaterLevel.DataAccess
{
	...
	public class ManufactureTableAdapter : BaseDataAdapter
	{
		
		
		#region Constructers

			public ManufactureTableAdapter():base()
            {
                InitInsertCommand();
                InitUpdateCommand();
                InitDeleteCommand();
		...
		protected  void InitInsertCommand()
		{
			...
			InsertCommand.CommandText = @"INSERT INTO Manufacture(ManufactureName,Status) VALUES (@ManufactureName,@Status);SELECT ID FROM Manufacture WHERE (ID = SCOPE_IDENTITY())";
						InsertCommand.Parameters.Add(new SqlParameter("@ManufactureName", SqlDbType.VarChar, 100,"ManufactureName"));
		...
		protected  void InitUpdateCommand()
		{
			...
			UpdateCommand.CommandText = @"UPDATE Manufacture SET ManufactureName = @ManufactureName, Status = @Status WHERE (ID = @ID)";
			...
			UpdateCommand.Parameters.Add(new SqlParameter("@ManufactureName", SqlDbType.VarChar, 100,"ManufactureName"));
		...
		protected  void InitDeleteCommand()
		{
			...
			DeleteCommand.CommandText = "DELETE FROM Manufacture WHERE (ID = @ID)";
		...
		protected  void InitCommandCollection()
		{
			...
			((SqlCommand) (CommandCollection[0])).CommandText = "SELECT ID,ManufactureName,Status FROM Manufacture";
            ...
            ((SqlCommand)(CommandCollection[1])).CommandText = "SELECT ID,ManufactureName,Status FROM Manufacture WHERE ID =  @ID";
            ...
            ((SqlCommand)(CommandCollection[2])).CommandText = "SELECT ID,ManufactureName,Status FROM Manufacture WHERE Status =  @Status";
		...
		public DataSet GetManufactureByStatus(string status)
		{
		...
		{
	InsertCommand.Parameters["@ManufactureName"].Value = manufacture.ManufactureName;
	InsertCommand.Parameters["@Status"].Value = manufacture.Status;
		...
		{
	UpdateCommand.Parameters["@ID"].Value = manufacture.ID;
	UpdateCommand.Parameters["@ManufactureName"].Value = manufacture.ManufactureName;
	UpdateCommand.Parameters["@Status"].Value = manufacture.Status;
        ...
        private Manufacture DataTableToEntity(DataTable dt)
        {
            ...
            if(Null.IsNotNull(dt)==true && dt.Rows.Count > 0)
            {
                ...
                if (Null.IsNotNull(dt.Rows[0]))
                {
					...
					{
						manufacture.ID=Convert.ToInt32(dr["ID"]);
					}
					else
					{
						manufacture.ID=0;
					}
					if (Null.IsNotNull(dr["ManufactureName"]))
					{
						manufacture.ManufactureName= Convert.ToString(dr["ManufactureName"]);
					}
					else
					{
						manufacture.ManufactureName=string.Empty;
					}
					if (Null.IsNotNull(dr["Status"]))
					{
						manufacture.Status= Convert.ToString(dr["Status"]);
					}
					else
					{
						manufacture.Status=string.Empty;
					}

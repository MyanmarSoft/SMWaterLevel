...
namespace SMWaterLevel.DataAccess
{
	...
	public class SpecialityTableAdapter : BaseDataAdapter
	{
		
		
		#region Constructers

			public SpecialityTableAdapter():base()
            {
                InitInsertCommand();
                InitUpdateCommand();
                InitDeleteCommand();
		...
		protected  void InitInsertCommand()
		{
			...
			InsertCommand.CommandText = @"INSERT INTO Speciality(SpecialityName,Status) VALUES (@SpecialityName,@Status);SELECT ID FROM Speciality WHERE (ID = SCOPE_IDENTITY())";
						InsertCommand.Parameters.Add(new SqlParameter("@SpecialityName", SqlDbType.VarChar, 50,"SpecialityName"));
		...
		protected  void InitUpdateCommand()
		{
			...
			UpdateCommand.CommandText = @"UPDATE Speciality SET SpecialityName = @SpecialityName, Status = @Status WHERE (ID = @ID)";
			...
			UpdateCommand.Parameters.Add(new SqlParameter("@SpecialityName", SqlDbType.VarChar, 50,"SpecialityName"));
		...
		protected  void InitDeleteCommand()
		{
			...
			DeleteCommand.CommandText = "DELETE FROM Speciality WHERE (ID = @ID)";
		...
		protected  void InitCommandCollection()
		{
			...
			((SqlCommand) (CommandCollection[0])).CommandText = "SELECT ID,SpecialityName,Status FROM Speciality";
            ...
            ((SqlCommand)(CommandCollection[1])).CommandText = "SELECT ID,SpecialityName,Status FROM Speciality WHERE ID =  @ID";
            ...
            ((SqlCommand)(CommandCollection[2])).CommandText = "SELECT ID,SpecialityName,Status FROM Speciality WHERE Status =  @Status";
		...
		public DataSet GetSpecialityByStatus(string status)
		{
	...
	InsertCommand.Parameters["@Status"].Value = speciality.Status;
		...
		{
	UpdateCommand.Parameters["@ID"].Value = speciality.ID;
	UpdateCommand.Parameters["@SpecialityName"].Value = speciality.SpecialityName;
	UpdateCommand.Parameters["@Status"].Value = speciality.Status;
        ...
        private Speciality DataTableToEntity(DataTable dt)
        {
            ...
            if(Null.IsNotNull(dt)==true && dt.Rows.Count > 0)
            {
                ...
                if (Null.IsNotNull(dt.Rows[0]))
                {
					...
					{
						speciality.ID=Convert.ToInt32(dr["ID"]);
					}
					else
					{
						speciality.ID=0;
					}
					if (Null.IsNotNull(dr["SpecialityName"]))
					{
						speciality.SpecialityName= Convert.ToString(dr["SpecialityName"]);
					}
					else
					{
						speciality.SpecialityName=string.Empty;
					}
					if (Null.IsNotNull(dr["Status"]))
					{
						speciality.Status= Convert.ToString(dr["Status"]);
					}
					else
					{
						speciality.Status=string.Empty;
					}

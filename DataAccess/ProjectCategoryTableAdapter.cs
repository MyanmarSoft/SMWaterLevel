...
namespace SMWaterLevel.DataAccess
{
	...
	public class ProjectCategoryTableAdapter : BaseDataAdapter
	{
		
		
		#region Constructers

			public ProjectCategoryTableAdapter():base()
            {
                InitInsertCommand();
                InitUpdateCommand();
                InitDeleteCommand();
		...
		protected  void InitInsertCommand()
		{
			...
			InsertCommand.CommandText = @"INSERT INTO ProjectCategory(ProjectCategoryName,Status) VALUES (@ProjectCategoryName,@Status);SELECT ID FROM ProjectCategory WHERE (ID = SCOPE_IDENTITY())";
		...
		protected  void InitUpdateCommand()
		{
			...
			UpdateCommand.CommandText = @"UPDATE ProjectCategory SET ProjectCategoryName = @ProjectCategoryName, Status = @Status WHERE (ID = @ID)";
		...
		protected  void InitDeleteCommand()
		{
			...
			DeleteCommand.CommandText = "DELETE FROM ProjectCategory WHERE (ID = @ID)";
		...
		protected  void InitCommandCollection()
		{
			...
			((SqlCommand) (CommandCollection[0])).CommandText = "SELECT ID,ProjectCategoryName,Status FROM ProjectCategory";
            ...
            ((SqlCommand)(CommandCollection[1])).CommandText = "SELECT ID,ProjectCategoryName,Status FROM ProjectCategory WHERE ID =  @ID";
            ...
            ((SqlCommand)(CommandCollection[2])).CommandText = "SELECT ID,ProjectCategoryName,Status FROM ProjectCategory WHERE Status =  @Status";
		...
		public DataSet GetProjectCategoryByStatus(string status)
		{
	...
	InsertCommand.Parameters["@Status"].Value = projectCategory.Status;
		...
		{
	UpdateCommand.Parameters["@ID"].Value = projectCategory.ID;
	UpdateCommand.Parameters["@ProjectCategoryName"].Value = projectCategory.ProjectCategoryName;
	UpdateCommand.Parameters["@Status"].Value = projectCategory.Status;
        ...
        private ProjectCategory DataTableToEntity(DataTable dt)
        {
            ...
            if(Null.IsNotNull(dt)==true && dt.Rows.Count > 0)
            {
                ...
                if (Null.IsNotNull(dt.Rows[0]))
                {
					...
					{
						projectCategory.ID=Convert.ToInt32(dr["ID"]);
					}
					else
					{
					...
					{
						projectCategory.ProjectCategoryName= Convert.ToString(dr["ProjectCategoryName"]);
					}
					else
					{
					...
					{
						projectCategory.Status= Convert.ToString(dr["Status"]);
					}
					else
					{
						projectCategory.Status=string.Empty;
					}

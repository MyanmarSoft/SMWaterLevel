...
namespace SMWaterLevel.DataAccess
{
	...
	public class ItemCategoryTableAdapter : BaseDataAdapter
	{
		
		
		#region Constructers

			public ItemCategoryTableAdapter():base()
            {
                InitInsertCommand();
                InitUpdateCommand();
                InitDeleteCommand();
		...
		protected  void InitInsertCommand()
		{
			...
			InsertCommand.CommandText = @"INSERT INTO ItemCategory(ItemCategoryName,Status) VALUES (@ItemCategoryName,@Status);SELECT ID FROM ItemCategory WHERE (ID = SCOPE_IDENTITY())";
						InsertCommand.Parameters.Add(new SqlParameter("@ItemCategoryName", SqlDbType.NVarChar, 100,"ItemCategoryName"));
		...
		protected  void InitUpdateCommand()
		{
			...
			UpdateCommand.CommandText = @"UPDATE ItemCategory SET ItemCategoryName = @ItemCategoryName, Status = @Status WHERE (ID = @ID)";
			...
			UpdateCommand.Parameters.Add(new SqlParameter("@ItemCategoryName", SqlDbType.NVarChar, 100,"ItemCategoryName"));
		...
		protected  void InitDeleteCommand()
		{
			...
			DeleteCommand.CommandText = "DELETE FROM ItemCategory WHERE (ID = @ID)";
		...
		protected  void InitCommandCollection()
		{
			...
			((SqlCommand) (CommandCollection[0])).CommandText = "SELECT ID,ItemCategoryName,Status FROM ItemCategory";
            ...
            ((SqlCommand)(CommandCollection[1])).CommandText = "SELECT ID,ItemCategoryName,Status FROM ItemCategory WHERE ID =  @ID";
            ...
            ((SqlCommand)(CommandCollection[2])).CommandText = "SELECT ID,ItemCategoryName,Status FROM ItemCategory WHERE Status =  @Status";
		...
		public DataSet GetItemCategoryByStatus(string status)
		{
		...
		{
	InsertCommand.Parameters["@ItemCategoryName"].Value = itemCategory.ItemCategoryName;
	InsertCommand.Parameters["@Status"].Value = itemCategory.Status;
		...
		{
	UpdateCommand.Parameters["@ID"].Value = itemCategory.ID;
	UpdateCommand.Parameters["@ItemCategoryName"].Value = itemCategory.ItemCategoryName;
	UpdateCommand.Parameters["@Status"].Value = itemCategory.Status;
        ...
        private ItemCategory DataTableToEntity(DataTable dt)
        {
            ...
            if(Null.IsNotNull(dt)==true && dt.Rows.Count > 0)
            {
                ...
                if (Null.IsNotNull(dt.Rows[0]))
                {
					...
					if (Null.IsNotNull(dr["ItemCategoryName"]))
					{
						itemCategory.ItemCategoryName= Convert.ToString(dr["ItemCategoryName"]);
					}
					else
					{
						itemCategory.ItemCategoryName=string.Empty;
					}
					if (Null.IsNotNull(dr["Status"]))
					{
						itemCategory.Status= Convert.ToString(dr["Status"]);
					}
					else
					{
						itemCategory.Status=string.Empty;
					}

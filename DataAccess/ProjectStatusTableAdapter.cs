...
namespace SMWaterLevel.DataAccess
{
	...
	public class ProjectStatusTableAdapter : BaseDataAdapter
	{
		
		
		#region Constructers

			public ProjectStatusTableAdapter():base()
            {
                InitInsertCommand();
                InitUpdateCommand();
                InitDeleteCommand();
		...
		protected  void InitInsertCommand()
		{
			...
			InsertCommand.CommandText = @"INSERT INTO ProjectStatus(ProjectStatusName,Status) VALUES (@ProjectStatusName,@Status);SELECT ID FROM ProjectStatus WHERE (ID = SCOPE_IDENTITY())";
						InsertCommand.Parameters.Add(new SqlParameter("@ProjectStatusName", SqlDbType.VarChar, 50,"ProjectStatusName"));
		...
		protected  void InitUpdateCommand()
		{
			...
			UpdateCommand.CommandText = @"UPDATE ProjectStatus SET ProjectStatusName = @ProjectStatusName, Status = @Status WHERE (ID = @ID)";
			...
			UpdateCommand.Parameters.Add(new SqlParameter("@ProjectStatusName", SqlDbType.VarChar, 50,"ProjectStatusName"));
		...
		protected  void InitDeleteCommand()
		{
			...
			DeleteCommand.CommandText = "DELETE FROM ProjectStatus WHERE (ID = @ID)";
		...
		protected  void InitCommandCollection()
		{
			...
			((SqlCommand) (CommandCollection[0])).CommandText = "SELECT ID,ProjectStatusName,Status FROM ProjectStatus";
            ...
            ((SqlCommand)(CommandCollection[1])).CommandText = "SELECT ID,ProjectStatusName,Status FROM ProjectStatus WHERE ID =  @ID";
            ...
            ((SqlCommand)(CommandCollection[2])).CommandText = "SELECT ID,ProjectStatusName,Status FROM ProjectStatus WHERE Status =  @Status";
		...
		public DataSet GetProjectStatusByStatus(string status)
		{
		...
		public int Insert(ProjectStatus projectStatus)
		{
	InsertCommand.Parameters["@ProjectStatusName"].Value = projectStatus.ProjectStatusName;
	InsertCommand.Parameters["@Status"].Value = projectStatus.Status;
		...
		public int Update(ProjectStatus projectStatus)
		{
	UpdateCommand.Parameters["@ID"].Value = projectStatus.ID;
	UpdateCommand.Parameters["@ProjectStatusName"].Value = projectStatus.ProjectStatusName;
	UpdateCommand.Parameters["@Status"].Value = projectStatus.Status;
        ...
        private ProjectStatus DataTableToEntity(DataTable dt)
        {
            ...
            if(Null.IsNotNull(dt)==true && dt.Rows.Count > 0)
            {
                ...
                if (Null.IsNotNull(dt.Rows[0]))
                {
					...
					{
						projectStatus.ID=Convert.ToInt32(dr["ID"]);
					}
					else
					{
						projectStatus.ID=0;
					}
					if (Null.IsNotNull(dr["ProjectStatusName"]))
					{
						projectStatus.ProjectStatusName= Convert.ToString(dr["ProjectStatusName"]);
					}
					else
					{
						projectStatus.ProjectStatusName=string.Empty;
					}
					if (Null.IsNotNull(dr["Status"]))
					{
						projectStatus.Status= Convert.ToString(dr["Status"]);
					}
					else
					{
						projectStatus.Status=string.Empty;
					}
